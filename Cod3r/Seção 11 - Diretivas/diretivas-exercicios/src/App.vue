<template>
  <div id="app">
    <h1>Diretivas</h1>
    <hr />
    <p
      v-destaque-local:fundo.atrasar.alternar="{cor1: 'green', cor2: 'red', atraso: 1000, intervalo: 500}"
    >Usando diretiva personalizada</p>
    <p v-destaque="cor">Usando diretiva personalizada</p>

    <!-- <p v-test:argumento.modificador="valor">Usando diretiva personalizada</p> -->
    <hr />
    <p v-text="'Usando diretiva v-text'"></p>
    <p v-html="'Usando diretiva <strong> v-text </strong>'"></p>
    <hr />
  </div>
</template>

<script>
export default {
  data() {
    return {
      cor: "red",
    };
  },
  //Declara uma diretiva localmente
  directives: {
    "destaque-local": {
      bind(el, binding, vnode) {
        const aplicarCor = (cor) => {
          if (binding.arg === "fundo") {
            el.style.backgroundColor = cor;
          } else {
            el.style.color = binding.value.cor1;
          }
        };
        const cor1 = binding.value.cor1;
        const cor2 = binding.value.cor2;
        let corAtual = cor1;
        let atraso = 0;
        if (binding.modifiers["atrasar"]) atraso = binding.value.atraso;

        setTimeout(() => {
          if (binding.modifiers["alternar"]) {
            setInterval(() => {
              corAtual = corAtual === cor1 ? cor2 : cor1;
              aplicarCor(corAtual);
            }, binding.value.intervalo);
          } else {
            aplicarCor(binding.value.cor1);
          }
        }, atraso);
      },
    },
  },
};
</script>

<style>
#app {
  font-family: "Avenir", Helvetica, Arial, sans-serif;
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
  text-align: center;
  color: #2c3e50;
  margin-top: 60px;
  font-size: 2.5rem;
}
</style>
